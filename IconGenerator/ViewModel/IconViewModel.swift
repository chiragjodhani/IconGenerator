...
class IconViewModel: ObservableObject {
    ...
    @Published var iconSizes: [Int] = [20, 60, 58, 87, 80, 120, 180, 40, 29, 76, 152, 167, 1024, 16, 32, 64, 128, 256, 512, 1024]
        ...
        folderSelector { folderURL in
            ...
            DispatchQueue.global(qos: .userInteractive).async {
                do {
                    ...
                    // 3. Writing Content.json file inside the folder.
                    self.writeContentsFile(folderURL: modifiedURL.appendingPathComponent("Contents.json"))
                    
                    // 4. Generating Icon Set Writing inside the folder..
                    if let pickedImage = self.pickedImage {
                        self.iconSizes.forEach { size in
                            ...
                            pickedImage.resizeImage(size: imageSize).writeImage(to: imageURL)
                        }
